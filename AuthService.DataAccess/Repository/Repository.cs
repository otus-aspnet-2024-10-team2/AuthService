...
public abstract class Repository<T, TPrimaryKey>(DbContext dbContext) : IRepository<T, TPrimaryKey> where T 
        : class, IEntity<TPrimaryKey>
{
    ...
    {
        return _entitySet.Find(id) ?? throw new ArgumentNullException($"Сущность с {id} не найдена");
    }
    ...
    {
        return await _entitySet.FindAsync([id], cancellationToken: cancellationToken) ?? throw new ArgumentNullException($"Сущность с {id} не найдена");
    }
